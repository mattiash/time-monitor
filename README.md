...
```bash
docker run --rm ghcr.io/mattiash/time-monitor
```
...
```bash
docker run --rm -e NTP_SERVERS="pool.ntp.org time.google.com" ghcr.io/mattiash/time-monitor
```
...
docker run --rm ghcr.io/mattiash/time-monitor --interval 60 --sample 5
```
